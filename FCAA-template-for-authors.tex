...
\begin{problem}
	Derive conditions on the parameters $(k_p,k_\eta,\alpha)$ analytically such that the fractional order controller:
	\begin{equation}
	C(s)=k_p+k_\eta s^\alpha, \label{eqcontrol}
	\end{equation}
	BIBO-stabilizes the closed-loop plant in Fig.~\ref{GCDiagram} described by the transfer function in Eq.~\eqref{sys1}.
...
%\end{proof}
\begin{definition}[Frequency crossing set]
	...
	\end{equation}
\end{definition}
\begin{definition}[Stability root boundaries]
...
\begin{proposition} \label{prop:IRB}
	...
	%\begin{multline}
	%\tilde{\Delta}(s)\!\!=\!\!a_ns^n \!+\!\mathcal{O}(n-1)\!+\!\sqrt{\alpha_{\tilde{q}}}{s}^{\frac{\tilde{q}}{2}}\mathcal{O}(1) \\+(k_p\!+\!k_ds^\mu)\left(b_ms^m+\mathcal{O}(m-1)+\sqrt{\beta_{\tilde{p}}}{s}^{\frac{\tilde{p}}{2}}\mathcal{O}(1)\right), \label{eqch1a}
	%\end{multline}
	%where 
...
\begin{proof}
	...
	%
	%
	%which is the distance between the point $\mathbf{k}^*$ and the RRB, the point $\mathbf{k}$ in which $\xi(0)$ is globally a minimum, is given by the solution of $\tfrac{d\xi^2(0)}{dk_d}=0$.
	%Then, this minimal value is $d_0$ and is computed by substituting the solution into equation \eqref{eqfragRRB}.
	%
	%
	%According to the fact that $\mathbf{k}^*$ belongs to a stability region which is delimited by a stability crossing boundary, it is a fact that the system will loose its stability if the controller $\mathbf{k}^*$ has a parameter deviation such that it crosses for at least on of its boundaries. Therefore, the goal will be to compute the minimal distance between a point $\mathbf{k}^*$ and the different boundaries of the stability region. \par 
	%
	%To compute the minimal distance between a point $\mathbf{k}^*$ and any of the stability root boundaries let us start with the CRB described in Proposition \ref{prop:CRB} which is defined for $\omega\neq0$. Here, we need to identify the points $\mathbf{k}(\omega)$ in which the tangent vectors to the curve are orthogonal to $\mathbf{k}(\omega)-\mathbf{k}^*$, and such values are given by the roots of \eqref{eqfCRB}. Hence, in such a situation, the minimum distance is computed by $d_\ell$ in  \eqref{eqfragility}. 
	%
	...
	%\end{equation}
	%which is the distance between the point $\mathbf{k}^*$ and the RRB, the point $\mathbf{k}$ in which $\xi(0)$ is globally a minimum, is given by the solution of $\tfrac{d\xi^2(0)}{dk_d}=0$.
	%Then, this minimal value is $d_0$ and is computed by substituting the solution into equation \eqref{eqfragRRB}. In a similar manner we can obtain the distance $d_\infty$ which corresponds to the minimal distance between $\mathbf{k}^*$ and the IRB related to $\omega=\infty$ and described in Proposition \ref{prop:IRB} by  substituting each of the cases shown in Eqs. \eqref{eqIRBa} and \eqref{eqIRBb} in \eqref{eqfragilitya}, leads to
	%\begin{equation}
	%   \xi(\infty)=\sqrt{(k_p-k_p^*)^2+(\ell+k_d^*)^2}, \label{eqfragilityb}
	%\end{equation}
	%where $\ell$ is defined in \eqref{eqFa} and \eqref{eqFb}. Then, \eqref{eqfragilityb} is the distance between the point $\mathbf{k}^*$ and the IRB, the point $\mathbf{k}$ in which $\xi(\infty)$ is globally a minimum is given by the solution of $\tfrac{d\xi^2(\infty)}{dk_p}=0$, when this solution is substituted in \eqref{eqfragilityb} we obtain the minimal distance $d_\infty$. Finally, the minimal distance $d$ can be computed by using \eqref{eqdistance}. 
\end{proof}
